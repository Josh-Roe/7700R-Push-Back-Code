...
inline uint32_t skillsStartTime = 0;
inline bool skillsSequenceActive = false;

inline uint32_t skillsSlowStartTime = 0;
inline bool skillsSlowSequenceActive = false;

inline ScoringMode requestedMode = ScoringMode::NONE;

constexpr int TOP_SLOWED_VOLTAGE = 200; // change this if you want slower/faster
...
inline void applyScoringMode(ScoringMode mode) {
  ...
  case ScoringMode::SKILLS:
    topStage.set_brake_mode(MOTOR_BRAKE_COAST);
    bottomStageFull.move_voltage(9000);
    bottomStageHalf.move_voltage(9000);
    topStage.move_voltage(-6000);
    hoodPiston.set_value(true);
    break;
...
inline void setScoringMode(const std::string &mode) {
  ...
  if (requestedMode == ScoringMode::SKILLS &&
      previousMode != ScoringMode::SKILLS) {
    skillsSequenceActive = true;
    skillsStartTime = pros::millis();
  }
  ...
  if (requestedMode == ScoringMode::SKILLSSLOW &&
      previousMode != ScoringMode::SKILLSSLOW) {
    skillsSlowSequenceActive = true;
    skillsSlowStartTime = pros::millis();
  }
...
inline void notifyIntakeReleased() { intakeTopStoppedByTorque = false; }
...
inline void scoring_task(void * /*param*/) {
  ...
  while (true) {
    ...
    bool jamDetectionEnabled = (requestedMode == ScoringMode::INTAKE);

    bool jamNow =
        jamDetectionEnabled && allowJamDetect &&
...
} else if (requestedMode == ScoringMode::TOPSLOW) {
  if (topDelayActive) {
    topStage.move_voltage(0);
  } else {
    topStage.move_voltage(6000);
  ...
  topStage.move_voltage(0);
    ...
    case JamState::IDLE: {
      ...
      if (topDelayActive) {
        ...
        case ScoringMode::MIDDLE:
          if (middleSequenceActive &&
              (pros::millis() - middleStartTime < 100)) {
