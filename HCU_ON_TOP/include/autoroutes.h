...
inline void left7Auto() {
  ...
  pros::delay(400);
  scraper_tog();
  chassis.turnToHeading(170, 600, {.minSpeed = 80, .earlyExitRange = 10});
  chassis.moveToPoint(79, -48, 900);
  chassis.moveToPoint(79, -46, 820, {.maxSpeed = 45});
  chassis.moveToPoint(20, -49, 1000, {.forwards = false, .maxSpeed = 90});
  pros::delay(1000);
  setScoringMode("TOP");
...
inline void right7Auto() {
  ...
  chassis.moveToPoint(79, 54, 700);
  chassis.moveToPoint(79, 52, 900, {.maxSpeed = 45, .minSpeed = 45});
  chassis.moveToPoint(20, 52, 1000, {.forwards = false, .maxSpeed = 100});
  pros::delay(1000);
  setScoringMode("TOP");
  chassis.moveToPoint(0, 40, 200, {.forwards = false, .maxSpeed = 40});
...
inline void fullWPAuto() {
  ...
  chassis.setPose({50, 15.0, 0});
    startFixPoseTask();
    wing_tog();
  // Full WP AUTO GOES HERE
  moveDistance(28.5, 850, 110);
  chassis.turnToHeading(89, 600);
  pros::delay(200);
  scraper_tog();
  setScoringMode("INTAKE");
  pros::delay(400);
  moveDistance(100000, 920, 40);
  chassis.moveToPoint(20, 48, 1000, {.forwards = false, .maxSpeed = 90});
  pros::delay(800);
  setScoringMode("TOP");
  chassis.moveToPoint(0, 48, 650, {.forwards = false, .maxSpeed = 40});
  scraper_tog();
  pros::delay(650);
  ...
  chassis.moveToPoint(20, -46.0, 1100, {.forwards = false, .maxSpeed = 90});
  pros::delay(700);
  setScoringMode("TOP");
  pros::delay(700);
  setScoringMode("OUTTAKE");
  pros::delay(250);
  setScoringMode("INTAKE");
  chassis.moveToPoint(90, -47.0, 600, {.maxSpeed = 80});
  setScoringMode("NONE");
  chassis.setPose({30, -48, chassis.getPose().theta});
  chassis.moveToPoint(90, -47.0, 750, {.maxSpeed = 40});
  setScoringMode("INTAKE");
...
inline void leftElimsAuto() {
  ...
  moveDistance(8, 800);
  setScoringMode("INTAKE");
  chassis.turnToPoint(19.5,-22,650);
  chassis.moveToPoint(19.5, -22, 1400, {.minSpeed = 40});
  pros::delay(400);
  scraper_tog();
  chassis.turnToPoint(17.5, -14, 700, {.forwards = false});
  pros::delay(700);
  moveDistance(-18.5, 800);
  setScoringMode("SKILLS");
  pros::delay(1000);
  scraper_tog();
  moveDistance(3, 500, 127, 40);
  pros::delay(500);
  setScoringMode("NONE");
  pros::delay(1300);
  moveDistance(-13, 600, 70, 70);
  chassis.setPose({10, -11, chassis.getPose().theta});
...
// // //SCORE 2 MIDDLE
  wing_tog();
  setScoringMode("INTAKE");
  chassis.moveToPoint(-24 , 19.5, 1600, {.maxSpeed = 40});
  pros::delay(1600);
  moveDistance(-3, 700, 127, 40);
  chassis.turnToPoint(-10, 12, 800, {.forwards = false});
  chassis.moveToPoint(-10, 12, 800, {.forwards = false});
  // chassis.swingToPoint(-2.5,12.5, lemlib::DriveSide::RIGHT, 2000 ,{.forwards = false, .maxSpeed = 80});
  // chassis.moveToPoint(-2.5,12.5, 1600, {.forwards = false, .maxSpeed = 80});
  chassis.swingToPoint(-4, 0, lemlib::DriveSide::LEFT, 800, {.forwards = false, .maxSpeed = 80});
  pros::delay(800);
  moveDistance(-5, 600);
...
pros::delay(600);
  ...
  chassis.setPose({chassis.getPose().x, 72 - float(distance_sensor_right.get()/25.4 + 9.0), chassis.getPose().theta});
  ...
  chassis.moveToPoint(64, 20, 1700, {.maxSpeed = 70});
chassis.moveToPoint(78, -100, 1000, {.maxSpeed = 100});
preroller_tog();
  setScoringMode("INTAKE");
chassis.moveToPoint(78, -100, 1300, {.maxSpeed = 70});
  ...
  resetOnPark();
  pros::delay(100);
  chassis.moveToPoint(24, -17, 1600, {.maxSpeed = 90});
  setScoringMode("INTAKE");
  ...
  chassis.moveToPoint(-84, 100, 1200, {.maxSpeed = 120});
  preroller_tog();
  setScoringMode("OUTTAKE");
  scraper_tog();
  pros::delay(250);
