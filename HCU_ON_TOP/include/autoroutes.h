...
inline void leftQualsAuto() {
  ...
  chassis.moveToPoint(30, -48.5, 1400, {.forwards = false, .maxSpeed = 80});
  pros::delay(900);
  setScoringMode("TOP");
  chassis.moveToPoint(0, -48.5, 800, {.forwards = false, .maxSpeed = 40});
  scraper_tog();
  pros::delay(1500);
...
inline void fullWPAuto() {
  ...
  chassis.turnToPoint(26,23,800);
  chassis.moveToPoint(26, 23, 1400, {.minSpeed = 40});
  pros::delay(500);
  scraper_tog();
  chassis.moveToPoint(30, -29, 2000, {.minSpeed = 40});
  pros::delay(300);
  scraper_tog();
  pros::delay(550);
  scraper_tog();
  chassis.moveToPoint(16, -4, 1300, {.forwards = false, .maxSpeed = 100});
  pros::delay(800);
  setScoringMode("MIDDLE");
  pros::delay(700);
  setScoringMode("NONE");
  chassis.moveToPoint(52, -45, 1500, {.maxSpeed = 100});
  chassis.turnToHeading(88, 600);
  setScoringMode("INTAKE");
  pros::delay(600);
  moveDistance(100000, 1100, 40);
  chassis.moveToPoint(20, -45, 1300, {.forwards = false, .maxSpeed = 80});
  pros::delay(1000);
  setScoringMode("TOP");
  chassis.moveToPoint(0, -45, 700, {.forwards = false, .maxSpeed = 40});
  scraper_tog();
  pros::delay(1000000);
}
inline void leftElimsAuto() {
  pros::lcd::print(6, "Running Left Elims Auto");
  chassis.setPose({50, -17.5, 180});
  startFixPoseTask();
...
//SCORE 2 MIDDLE
  ...
  chassis.turnToPoint(-11, 7, 800, {.forwards = false});
  setScoringMode("NONE");
  chassis.moveToPoint(-11, 7, 1200, {.forwards = false});
  pros::delay(400);
  setScoringMode("MIDDLE");
  pros::delay(1600);
  ...
  chassis.turnToPoint(32, 59, 1200, {.maxSpeed = 70});
  chassis.moveToPoint(32, 59, 1900, {.maxSpeed = 100});
  scraper_tog();
  setScoringMode("NONE");
  chassis.turnToHeading(90, 800);
  chassis.turnToPoint(42, 48, 1200);
  chassis.setPose({chassis.getPose().x, 72 - static_cast<float>(distance_sensor_left.get()/25.4) - 8.75f, chassis.getPose().theta});
  chassis.moveToPoint(42 , 48, 1500, {.minSpeed = 50});
  ...
  chassis.moveToPoint(90, 47, 700, { .maxSpeed = 80});
  chassis.moveToPoint(90, 47, 2600, { .maxSpeed= 35});
  ...
  chassis.turnToPoint(69, 0, 800);
  chassis.moveToPoint(69, -200, 1300, {.minSpeed = 127});
  setScoringMode("INTAKE");
  pros::delay(150);
  scraper_tog();
  pros::delay(500);
  scraper_tog();
  pros::delay(1000);
  chassis.moveToPoint(68, -200, 1500, {.maxSpeed = 90});
  ...
  setScoringMode("INTAKE");
  chassis.turnToPoint(14, -10, 800, {.forwards = false});
  chassis.moveToPoint(14, -10, 1200, {.forwards = false});
  pros::delay(400);
  setScoringMode("MIDDLE");
  pros::delay(2000);
  ...
  chassis.turnToPoint(-30, -61, 1200, {.maxSpeed = 70});
  chassis.moveToPoint(-30, -61, 1800, {.maxSpeed = 100});
  scraper_tog();
  setScoringMode("NONE");
  chassis.turnToHeading(-90, 800);
  chassis.setPose({chassis.getPose().x, -72 + static_cast<float>(distance_sensor_left.get()/25.4) + 8.75f, chassis.getPose().theta});
  chassis.turnToPoint(-44, -48, 1000);
  chassis.moveToPoint(-44, -48, 1500, {.minSpeed = 50});
  chassis.turnToPoint(-30, -48, 800, {.forwards = false});
  ...
  chassis.moveToPoint(-76, -42, 600, { .maxSpeed = 80});
  chassis.moveToPoint(-76, -42, 2200, { .maxSpeed = 30});
    setScoringMode("INTAKE");
  ...
  chassis.moveToPoint(-43 , -48, 1000);
  chassis.setPose({-30, -48, chassis.getPose().theta});
  chassis.moveToPoint(-65, -22, 1700, {.maxSpeed = 80});
  chassis.turnToPoint(-68, 0, 1000);
  chassis.moveToPoint(-68, 200, 1100, {.minSpeed = 127});
  setScoringMode("INTAKE");
  pros::delay(100);
  scraper_tog();
  pros::delay(500);
...
inline void leftRush() {
  ...
  scraper_tog();
  pros::delay(900);
  chassis.moveToPoint(36, -37, 1500);
  chassis.setPose({30, -48, chassis.getPose().theta});
  chassis.turnToPoint(11, -37, 800, {.forwards= false});
  setScoringMode("NONE");
  chassis.moveToPoint(11, -37, 1400, {.forwards = false});
  chassis.turnToHeading(105, 600);
  pros::delay(1000000);
}
